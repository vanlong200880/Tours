var defaults = {
  ...
  categorySlug : $("#category-slug").val()
};
...
//      console.log($("body .list-search").find('#province').val());
            ...
            $("body.travel .view-data").on('click', 'ul.load-post > li a', function () {
              ...
              $.fn.loadDetailPost(defaults['url'] + '/travel-view', id);
            });
            ...
            // view gallery
            $("body .popup-content").on('click', '.game-entertainment tr td .history', function () {
              var id = $(this).attr('data-id');
...
// load detail full page
        var mode = $.fn.getUrlParam('mode');
        if(mode == 'full'){
          $.fn.callSliderRoyalSlider();
          var id = $("#full-title").attr('data-id');
          $.fn.loadDetailPost(defaults['url'] + '/travel-view', id, mode);
        }
          },
          // load detail post
          $.fn.loadDetailPost = function(url,id, mode = ''){
              $.ajax({
                url: url,
                type: 'POST',
                dataType: "json",
  ...
  //            $("#page-comment").empty().append(data.htmlComment);
                    ...
                    $.fn.loadVideoByPostId(defaults['url'] + '/load-video-by-post-id', id, 1);
                    $('meta[name=description]').attr('content', 'abcs');
                    $('title').html('a sd sdfsdf');
                  }
...
//          $(".views-popup .views-container").addClass('open');
          ...
          $.fn.loadData = function (url, id,slug, category, nation, province, district, page, filter, sort, star, min, max, area, keyword, postId = '') {
            $.ajax({
              method: 'POST',
              url: url,
              data: {slug: slug, category: category, nation: nation, province: province, district: district, page: page, filter: filter, sort: sort, star: star, min: min, max: max, area: area, keyword: keyword, postId: postId},
              beforeSend: function () {
              ...
              success: function (data) {
                ...
                if($.fn.getUrlParam('mode') !='full')
                {
                  $.fn.initializeMap(data.dataJson, 'map', '');
                }
                
                if(postId){
                  $.fn.loadDetailPost(defaults['url'] + '/travel-view', postId,$.fn.getUrlParam('mode'));
                }
              }
            });
          },
...
//  $.fn.callAjaxRegion();
  ...
  $.fn.loadData(defaults['url'] + '/load-data', '#view-data ul.load-post', defaults['categorySlug'],defaults['categoryType'], defaults['nation'], defaults['province'], defaults['district'], page, filter, sort, star, min, max, area, keyword,postId);
  $.fn.render();
})(jQuery);
